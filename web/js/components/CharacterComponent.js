export class CharacterComponent {
  ...
  bindEvents() {
    ...
    const lvlButtons = container.querySelectorAll(
      "[data-action='modify-level']",
    );
    ...
    const statButtons = container.querySelectorAll(
      "[data-action='modify-stat']",
    );
  ...
  render(char) {
    ...
    document.getElementById("unspent-points").innerText =
      char.unspent_stat_points;

    // Render main numbers
    document.getElementById("char-hp").innerText =
      `${char.hp} / ${char.max_hp.total}`;
    document.getElementById("char-def").innerText = Number.isInteger(
      char.defense.total,
    )
      ? char.defense.total
      : char.defense.total.toFixed(2);
    document.getElementById("char-ap").innerText = Number.isInteger(
      char.ap.total,
    )
      ? char.ap.total
      : char.ap.total.toFixed(2);
    document.getElementById("char-stam").innerText = Number.isInteger(
      char.stamina.total,
    )
      ? char.stamina.total
      : char.stamina.total.toFixed(2);
