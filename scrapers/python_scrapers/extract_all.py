...
def main():
    sheets = {
        'sheet3.xml': 'bron_zasiegowa_raw.json',
        'sheet4.xml': 'tarcze_raw.json',
        'sheet5.xml': 'zbroje_raw.json',
        'sheet6.xml': 'rozne_raw.json'
    }
    
    with zipfile.ZipFile('Ekwipunki.xlsx', 'r') as z:
        ss = get_shared_strings(z)
        for xml_file, out_name in sheets.items():
            grid = sheet_to_grid(z, f'xl/worksheets/{xml_file}', ss)
            with open(out_name, 'w', encoding='utf-8') as f:
                json.dump(grid, f, ensure_ascii=False, indent=2)
            print(f"Extracted {xml_file} to {out_name} ({len(grid)} rows)")
